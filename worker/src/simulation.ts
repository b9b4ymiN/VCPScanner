import { portfolios, positions, dailySnapshots, alerts } from './schema'
...
export async function simulateEOD(env: Bindings, today: string): Promise<void> {
  const db = getDb(env.DB)

  // 1. Find active portfolio
  const portfolio = await db.select().from(portfolios)
    .where(eq(portfolios.status, 'active')).limit(1)
  if (!portfolio.length) return
  const port = portfolio[0]!

  // 2. Idempotency: skip if snapshot already exists for today
  ...
  for (const pos of openPositions) {
    ...
    let exitPrice: number = pos.entryPrice
    let exitReason: string = 'TIME'

    if (currentPrice !== null) {
      if (currentPrice <= pos.stopPrice) {
        shouldExit = true; exitPrice = currentPrice; exitReason = 'SL'
      } else if (currentPrice >= pos.targetPrice) {
        shouldExit = true; exitPrice = currentPrice; exitReason = 'TP'
      }
    }

    // Time exit: 14 calendar days
    if (!shouldExit && calendarDays >= 14) {
      if (currentPrice !== null) {
        shouldExit = true; exitPrice = currentPrice; exitReason = 'TIME'
      } else {
    ...
    if (shouldExit) {
      ...
      await db.update(positions).set({
        exitDate: today, exitPrice, exitReason, pnl, pnlPct, status: 'closed'
  ...
  // 5. Re-count open positions after exits
  ...
  await db.update(portfolios).set({
    cashBalance, totalValue, totalPnL, totalPnLPct,
