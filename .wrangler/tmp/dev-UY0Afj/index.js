...
var YahooFetchProvider = class {
  ...
  async throttledFetch(url) {
    ...
    if (!res.ok) throw new Error(`Yahoo API ${res.status}: ${url}`);
    return res;
  }
  ...
  async fetchFundamentals(symbol, market) {
    const ysymbol = `${symbol}${market.yahooSuffix}`;
    const modules = "financialData,defaultKeyStatistics,summaryProfile,summaryDetail";
    const url = `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(ysymbol)}?modules=${modules}`;
    const empty = {
      epsGrowthYoY: null,
    ...
    try {
      ...
      return {
        epsGrowthYoY: ks.earningsQuarterlyGrowth != null ? Number(ks.earningsQuarterlyGrowth) : null,
        revenueGrowthYoY: fd.revenueGrowth != null ? Number(fd.revenueGrowth) : null,
        profitMargin: fd.profitMargins != null ? Number(fd.profitMargins) : null,
        roe: fd.returnOnEquity != null ? Number(fd.returnOnEquity) : null,
        institutionalHold: ks.heldPercentInstitutions != null ? Number(ks.heldPercentInstitutions) : null,
        insiderHold: ks.heldPercentInsiders != null ? Number(ks.heldPercentInsiders) : null,
        ...
        fiftyTwoWeekHigh: sd.fiftyTwoWeekHigh != null ? Number(sd.fiftyTwoWeekHigh) : null,
        fiftyTwoWeekLow: sd.fiftyTwoWeekLow != null ? Number(sd.fiftyTwoWeekLow) : null
      };
    } catch {
      return empty;
    }
...
app.get("/api/alerts", async (c) => {
  ...
  const conditions = level ? and(eq(alerts.date, date), gte(alerts.sepaScore, minScore), eq(alerts.alertLevel, level.toUpperCase())) : and(eq(alerts.date, date), gte(alerts.sepaScore, minScore));
...
async function processBatch(env2, msg) {
  ...
  await db.update(scanRuns).set({
    totalScanned: sql`coalesce(total_scanned, 0) + ${scanned}`,
    totalPassed: sql`coalesce(total_passed, 0) + ${passed}`
...
var src_default = {
  ...
  async queue(batch, env2, _ctx) {
    for (const msg of batch.messages) {
      try {
        await processBatch(env2, msg.body);
        msg.ack();
      } catch (err) {
...
// ../../../Users/bfipa/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../../Users/bfipa/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-gQMOY3/middleware-insertion-facade.js
...
// ../../../Users/bfipa/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-gQMOY3/middleware-loader.entry.ts
